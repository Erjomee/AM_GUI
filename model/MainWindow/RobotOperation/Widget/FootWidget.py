...
class FootWidget(QWidget):
    def __init__(self, main_window):
        """
        Widget for displaying robot feet with pressure sensors and force vectors.
        ...
        # Find the widget in the main window
        ...
        # Remove scroll bars and set rendering hints
        ...
        self.left_foot_pixmap_item.setPixmap(self.left_foot_pixmap.scaled(int(self.scene.width() * 0.35),
                                                                          int(self.scene.height() * 0.45)))
        ...
        self.right_foot_pixmap_item.setPixmap(self.right_foot_pixmap.scaled(int(self.scene.width() * 0.35),
                                                                            int(self.scene.height() * 0.45)))
    ...
    def init_sensor(self, is_permanent=True):
        """
        Initialize foot sensors by updating the pixmap with sensor information.

        Args:
            is_permanent (bool): Whether the sensor information should be permanent.
        """
        self.update_pixmap(self.LeftFootSensors, self.RightFootSensors, is_permanent)

    def foot_sensor(self, foot, activate=False):
        """
        Set foot sensors based on activation status.

        Args:
            foot (str): Name of the foot ("left_foot" or "right_foot").
            activate (bool): Whether to activate or deactivate the foot sensors.
        """
        ...
        center_point = PressurePoint(0, 0, 3, border_color=QtGui.QColor(255, 0, 0),
                                     gradient_center=QtGui.QColor(255, 0, 0))
    ...
    def update_pixmap(self, LeftFootPressurePoints=[], RightFootPressurePoints=[], is_permanent=False):
        """
        Update foot pixmap with new pressure points and draw vectors.

        Args:
            LeftFootPressurePoints (list): List of pressure points for the left foot.
            RightFootPressurePoints (list): List of pressure points for the right foot.
            is_permanent (bool): Whether the pressure points should be permanent.
        """
        # Clear old pressure points
        if not is_permanent:
            ...
            self.RightFootPressurePoints = RightFootPressurePoints
        ...
        for foot, foot_pixmap_item in self.foots_pixmap_item.items():
            ...
            if sum(point.get_pressure for point in ListOfFootsCoordsPressure.get(foot)) != 0 and len(
                    ListOfFootsCoordsPressure) != 0:
                if not is_permanent:
                    foot_pixmap_item.setOpacity(1)
                    self.foot_sensor(foot, activate=True)

                # Find foot origin point
                ...
                for point in ListOfFootsCoordsPressure.get(foot):
                    ...
                    radial_gradient.setColorAt(0, point.get_gradient_center)
                    radial_gradient.setColorAt(1, point.get_border_color)
                ...
                # Change opacity for foot in the air
    ...
    def drawVector(self, x_start, y_start, vector, value):
        """
        Draw a vector from a starting point with given direction and magnitude.

        Args:
            x_start (float): x-coordinate of the starting point.
            y_start (float): y-coordinate of the starting point.
            vector (PressurePoint): Vector representing direction and magnitude of the force.
            value (float): Magnitude value of the vector.
        """
    ...
    def drawArrowhead(self, start_point, end_point, color, width):
        ...
        Args:
            start_point (QPointF): Starting point of the arrowhead.
            end_point (QPointF): Ending point of the arrowhead.
            color (QColor): Color of the arrowhead.
            width (float): Width of the arrowhead.
        """
    ...
    def clearItems(self, type_Item):
        """
        Clear items of a specific type from the scene.

        Args:
            type_Item (type): Type of items to clear (e.g., QGraphicsEllipseItem).
        """
    ...
    def resizeEvent(self, event):
        """
        Handle resize event of the widget.
